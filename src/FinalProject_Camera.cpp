...
int main(int argc, const char *argv[])
{
    ...
    std::vector<double> ttcCameraVec;
  	...
  	// Available  options: SHITOMASI, HARRIS, FAST, BRISK, ORB, AKAZE, SIFT
    string detectorType = "SIFT";

    // Available  options: BRISK, BRIEF, ORB, FREAK, AKAZE, SIFT
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
    {
        ...
        if(bVis)
        {
            show3DObjects((dataBuffer.end()-1)->boundingBoxes, cv::Size(4.0, 20.0), cv::Size(750, 500), true);
        }
        bVis = true;
